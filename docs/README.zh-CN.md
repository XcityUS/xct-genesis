...
WorldSeed 是多智能体世界的环境引擎。你定义场景、角色、规则、每个角色能看到什么。

跑起来之后，剩下的全交给 agent：决定做什么、和谁说话、欺骗、结盟、背叛。你可以从上方观察，私下 whisper 给任何 agent，或者下场扮演一个角色和它们一起玩。同一个引擎跑任何你定义的世界，每次运行都不一样，因为没人写过剧本。
