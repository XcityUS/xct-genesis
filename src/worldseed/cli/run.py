    ...
    *,
    ...
    prompt_by_agent = {
        agent_id: (
            ...
            "You are running one WorldSeed actor activation. Run "
            f"`uv run python {workspace}/ws.py perceive` first, choose one legal action "
            "from the returned `action_options`, check target state so you avoid "
            "finalized targets for open-target actions. If the action requires "
            "file references, create the required workspace-relative files at "
            "the paths described by the action schema and pass those paths in "
            "the action params. Then submit it with "
            f"`uv run python {workspace}/ws.py act ...`; use publish only for "
            "optional JSONL machine indexes. Work only under "
            f"{workspace}/agents/{agent_id}/ and by creating new shared files "
            f"under {workspace}/shared/ only when the action schema asks for "
            "them. Update status, then return a short summary, files produced "
            "if any, next intent, and blockers. "
            "Do not wait for the next tick unless MAIN explicitly asks."
        )
